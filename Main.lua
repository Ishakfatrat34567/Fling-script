...
Controller.CollisionConnections = {}
Controller.GuiConnections = {}
...
local function bindCollisionHandlers(character)
    for _, part in ipairs(getCharacterParts(character)) do
        connect(Controller.CollisionConnections, part.Touched, function(hit)
    ...
    connect(Controller.CollisionConnections, character.DescendantAdded, function(desc)
        if desc:IsA("BasePart") then
            connect(Controller.CollisionConnections, desc.Touched, function(hit)
                if not Controller.Enabled then
                    return
                end
...
local function refreshCharacterBinding()
    cleanupConnections(Controller.CollisionConnections)

    if LocalPlayer.Character then
        bindCollisionHandlers(LocalPlayer.Character)
    end

    connect(Controller.CollisionConnections, LocalPlayer.CharacterAdded, function(char)
        task.wait(0.2)
...
local function createGui()
    ...
    connect(Controller.GuiConnections, panel.InputBegan, function(input)
    ...
    connect(Controller.GuiConnections, UserInputService.InputChanged, function(input)
    ...
    connect(Controller.GuiConnections, toggleButton.MouseButton1Click, function()
        Controller.Enabled = not Controller.Enabled
        updateToggleVisual()
    end)

    connect(Controller.GuiConnections, toggleButton.MouseEnter, function()
        TweenService:Create(toggleButton, TweenInfo.new(0.12), {
            Size = UDim2.fromOffset(314, 38)
        }):Play()
    end)

    connect(Controller.GuiConnections, toggleButton.MouseLeave, function()
        TweenService:Create(toggleButton, TweenInfo.new(0.12), {
...
function Controller:Destroy()
    cleanupConnections(self.CollisionConnections)
    cleanupConnections(self.GuiConnections)
    if self.Gui then
        self.Gui:Destroy()
    end
end
